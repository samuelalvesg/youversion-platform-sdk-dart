// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'youversion_reader_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Portuguese (`pt`).
class YouVersionReaderLocalizationsPt extends YouVersionReaderLocalizations {
  YouVersionReaderLocalizationsPt([String locale = 'pt']) : super(locale);

  @override
  String get bibleFallbackTitle => 'Bíblia';

  @override
  String get fontSettingsTooltip => 'Configurações de fonte';

  @override
  String get fontSizeLabel => 'Tamanho da fonte';

  @override
  String get lineSpacingLabel => 'Espaçamento entre linhas';

  @override
  String get searchBooksHint => 'Pesquisar';

  @override
  String get introChipLabel => 'Introdução';

  @override
  String get previousChapterTooltip => 'Capítulo anterior';

  @override
  String get nextChapterTooltip => 'Próximo capítulo';

  @override
  String get decreaseFontSizeTooltip => 'Diminuir tamanho da fonte';

  @override
  String get increaseFontSizeTooltip => 'Aumentar tamanho da fonte';

  @override
  String get themeLabel => 'Tema';

  @override
  String get themePureWhite => 'Branco Puro';

  @override
  String get themeSepia => 'Sépia';

  @override
  String get themePaperGray => 'Cinza Papel';

  @override
  String get themeCream => 'Creme';

  @override
  String get themeMint => 'Menta';

  @override
  String get themeSkyBlue => 'Azul Céu';

  @override
  String get themeCharcoal => 'Carvão';

  @override
  String get themeMidnightBlue => 'Azul Meia-Noite';

  @override
  String get themeTrueBlack => 'Preto Absoluto';

  @override
  String get themeGraphite => 'Grafite';

  @override
  String get themeForestNight => 'Noite na Floresta';

  @override
  String get themePureContrastLight => 'Contraste Puro (Claro)';

  @override
  String get themePureContrastDark => 'Contraste Puro (Escuro)';

  @override
  String get initBoldLabel => 'Init Bold';

  @override
  String get initBoldFractionLabel => 'Intensidade do negrito';

  @override
  String get changeVersionTooltip => 'Alterar versão da Bíblia';

  @override
  String get tryAgainButton => 'Tentar novamente';

  @override
  String get rateLimitedError =>
      'Muitas requisições - aguarde um pouco antes de tentar de novo.';

  @override
  String get signInRequiredError =>
      'É preciso entrar na conta (ou sua sessão expirou).';

  @override
  String get notPermittedError => 'Não permitido para esta conta.';

  @override
  String get invalidResponseError => 'Resposta inesperada do servidor.';

  @override
  String get loadFailedError => 'Não foi possível carregar este capítulo.';
}
